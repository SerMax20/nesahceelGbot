        ...
        if len(message.command) > 1:
            if message.command[1] == "unzip":
                ...
                if file_upload is not None:
                    g_response = await upload_to_gdrive(file_upload, message)
                ...
                if file_uploade is not None:
                    gk_response = await upload_to_gdrive(file_uploade, message)
                 ...
                 if file_uploadg is not None:
                     gau_response = await upload_to_gdrive(file_uploadg, message)
                     LOGGER.info(gau_response)
        else:
            gaut_response = await upload_to_gdrive(the_real_download_location_g, message)
